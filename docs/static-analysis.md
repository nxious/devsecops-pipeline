...
`snyk` is available as a `npm` module for CLI usage. It can be installed using the command:
...
SonarQube is an open-source tool used to analyze source code and manage source code quality. It is split into multiple components, with a server being the base and additional tools used in sync with the server, in our case the [SonarQube Scanner](https://docs.sonarqube.org/latest/analysis/scan/sonarscanner/){target="_blank"}.
...
All the reports generated by various tools were stored in JSON format under the `reports` folder located in the default home directory of the Jenkins user at `/var/lib/jenkins`.